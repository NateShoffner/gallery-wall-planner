...
export function BatchAIProcessModal({ 
  ...
  const [results, setResults] = useState<ProcessResult[]>(
    pieceIds.map(id => ({ pieceId: id, status: 'pending' }))
  )
  
  // Process pieces sequentially
  useEffect(() => {
    if (currentIndex >= pieceIds.length) {
      // All done
      return
    }
    
    void processPiece(pieceIds[currentIndex])
  }, [currentIndex])
  
  async function processPiece(pieceId: string) {
    ...
    try {
      ...
      // Compress and call AI API
      const compressed = await compressImageForAI(file)
      const aiResult = await processImageWithAI(compressed)
  ...
  const isComplete = currentIndex >= pieceIds.length
      ...
      >
        ...
        <div className="px-6 py-4 border-b" style={{ borderColor: 'var(--border-normal)' }}>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
            Processing {pieceIds.length} piece{pieceIds.length !== 1 ? 's' : ''}
          </p>
        </div>
          ...
          <div className="relative h-2 rounded-full overflow-hidden mb-4" style={{ background: 'var(--bg-input)' }}>
            <div 
              ...
              style={{ 
                ...
                width: `${(currentIndex / pieceIds.length) * 100}%`,
              }}
            />
          </div>
          ...
          <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>
            {isComplete ? (
              ...
              <>
                ...
                Processing {currentIndex + 1} of {pieceIds.length}...
              </>
            )}
          </p>
